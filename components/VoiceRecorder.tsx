...
export function VoiceRecorder({ onClose, onSave }: VoiceRecorderProps) {
  ...
  const handleSave = () => {
    ...
    setPhase('saved')
    setTimeout(() => { onSave(blob, transcript); onClose() }, 1200)
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative z-10 w-full sm:max-w-md glass-card rounded-t-3xl sm:rounded-2xl p-8 shadow-2xl animate-fade-in-up">
        ...
        {(phase === 'idle' || phase === 'recording') && (
          <>
            <div className="text-center mb-6">
              <h2 className="text-xl font-bold text-foreground">Record Your Day</h2>
            ...
            <div className="text-center mb-4">
              <span className={`text-4xl font-mono font-bold tabular-nums ${
                phase === 'recording' ? 'text-accent' : 'text-muted-foreground/40'
              }`}>
            ...
            <p className="text-center text-xs text-muted-foreground">
              {phase === 'idle' ? 'Tap the mic to start' : 'Tap to stop recording'}
            </p>
          </>
        )}
        ...
        {phase === 'preview' && audioURL && (
          <>
            <div className="text-center mb-6">
              <h2 className="text-xl font-bold text-foreground">Preview Recording</h2>
              <p className="text-sm text-muted-foreground mt-1">
