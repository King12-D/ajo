...
import {
  Search, CheckCircle2, ShieldCheck, User, BarChart, TrendingUp, TrendingDown,
  Activity, ArrowRight
} from 'lucide-react'
...
function generateMockData(baseRev: number, score: number): TraderProfile['thirtyDayData'] {
  const data = []
  const now = new Date()
  for (let i = 29; i >= 0; i--) {
...
export default function LenderDashboard() {
  ...
  const handleSearch = () => {
    ...
    const foundKey = Object.keys(mockTraders).find(k => k.includes(q) || q.includes(k))
    if (foundKey) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-4 pb-20 pt-8 space-y-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10">
          <h1 className="text-3xl font-black text-foreground mb-3">Lender Portal</h1>
          <p className="text-muted-foreground mb-8">
            Query verified financial identities on-chain to assess risk and underwrite loans.
          </p>
          ...
          <div className="flex gap-2 justify-center mt-3">
            <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">Try:</span>
            {['8xKp', '7yLq', '9zMr'].map(t => (
              <button
                key={t}
                onClick={() => { setSearchQuery(t); setTimeout(() => handleSearch(), 100); }}
        ...
        {selectedTrader ? (
          <div className="space-y-6 animate-fade-in-up">
            ...
            <div className="glass-card rounded-2xl p-6 sm:p-8 flex flex-col md:flex-row gap-8 items-center md:items-start border border-border/60">
              ...
              <div className="shrink-0 flex flex-col items-center">
                {scoreUnlocked ? (
                  <AjoScore score={selectedTrader.ajoScore} size="lg" animated />
                ) : (
                  <div className="relative w-48 h-48 rounded-full border-[8px] border-muted flex items-center justify-center bg-card shadow-inner">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-full">
                      <span className="text-xl font-bold text-muted-foreground tracking-widest">LOCKED</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 w-full space-y-6">
                ...
                {scoreUnlocked ? (
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      { label: 'Consistency',        val: selectedTrader.consistency,       icon: <Activity className="w-4 h-4 text-accent" /> },
                      { label: 'Revenue Trend',      val: selectedTrader.revenueTrend,      icon: <TrendingUp className="w-4 h-4 text-primary" /> },
                      { label: 'Expense Discipline', val: selectedTrader.expenseDiscipline, icon: <TrendingDown className="w-4 h-4 text-secondary" /> },
                    ].map(stat => (
                      <div key={stat.label} className="bg-background/50 border border-border/40 rounded-xl p-4">
                        ...
                        <div className="w-full h-1.5 bg-muted rounded-full mt-3 overflow-hidden">
                          <div className="h-full bg-foreground rounded-full" style={{ width: \`\${stat.val}%\` }} />
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="bg-background/50 border border-border/40 rounded-xl p-6 text-center">
                    <p className="text-muted-foreground mb-4">
                      Unlock this trader's financial identity, Ajo Score, and 30-day historical transaction data.
                    </p>
                    <button
            ...
            {scoreUnlocked && (
              <div className="glass-card rounded-2xl p-6 sm:p-8 border border-border/60 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                <div className="flex items-center gap-2 mb-8">
                  <BarChart className="w-5 h-5 text-accent" />
                  <h3 className="font-bold text-lg text-foreground">30-Day Financial Trend</h3>
                </div>
                
                <div className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={selectedTrader.thirtyDayData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                      <defs>
          ...
          <div className="glass-card rounded-2xl p-12 text-center border border-border/60 max-w-2xl mx-auto mt-12">
            ...
            <p className="text-lg font-medium text-foreground mb-2">No Trader Selected</p>
            <p className="text-sm text-muted-foreground">
              Search for a trader's wallet address above to view their financial identity and Ajo Score.
            </p>
          </div>
        )}
      </main>
      ...
      {showPaymentModal && selectedTrader && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => paymentStatus === 'idle' && setShowPaymentModal(false)} />
          ...
          <div className="relative z-10 w-full sm:max-w-sm glass-card rounded-t-3xl sm:rounded-2xl p-8 shadow-2xl animate-fade-in-up border border-border/60">
            {paymentStatus === 'success' ? (
              <div className="text-center py-6 animate-fade-in-up">
                ...
                <h2 className="text-2xl font-black text-foreground mb-2">Payment Sent</h2>
                <p className="text-sm text-muted-foreground mb-8">
                  Successfully transferred 0.01 SOL.<br/>Score unlocked.
                </p>
              </div>
            ) : (
              <>
                <h2 className="text-2xl font-black text-foreground mb-6 text-center">Query Score</h2>
                
                <div className="bg-background/50 border border-border/40 rounded-xl p-5 mb-6 text-center">
                  <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1">Total Amount</p>
                  <p className="text-3xl font-mono font-black text-accent">0.01 SOL</p>
                  <p className="text-xs text-muted-foreground mt-2">~$1.45 USD</p>
                </div>
                ...
                >
                  {paymentStatus === 'processing' ? (
                    <span className="flex items-center gap-2">
                      <svg className="animate-spin -ml-1 mr-2 h-5 w-5 text-accent-foreground" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
